...
import { extractTableData, genInputParams } from "./helper";
...
class GenerateSP {
        ...
        AS
            ...
            ${
              this.table.is_deleted_field
                ? `UPDATE ${tableName} 
                    SET ISDELETED = 1
                    ${this.table.deleted_user_field ? ",DELETEDUSER = @DELETEDUSER" : ""}
                    ${this.table.deleted_date_field ? ",DELETEDDATE = getDate()" : ""}
  ...
  createOrUpdate() {
    ...
    return `
        ...
        AS
        BEGIN
            IF NOT EXISTS (SELECT TOP 1 1 FROM ${tableName} WHERE ${this.PR.field_name} = @${
      this.PR.field_name
    } ${this.table.is_deleted_field ? `AND ${this.table.is_deleted_field.field_name} = 0` : ""})
        BEGIN 
        INSERT INTO dbo.${tableName} 
        (${inputParams.map((field) => field.field_name).join(",")} ${
      this.table.created_date_field ? `,${this.table.created_date_field.field_name}` : ""
    })
                VALUES  (${inputParams.map((field) => `@${field.field_name}`).join(",")} ${
      this.table.created_date_field ? ", getDate()" : ""
        ...
        BEGIN 
            ...
            SET ${inputParams
              .filter((field) => field.field_name !== SPECIAL_FIELDS.CREATED_USER)
              .map(
                (field) => `${field.field_name} = ISNULL(@${field.field_name}, ${field.field_name})`
              )
              .join(",")} 
                ${
  ...
  getDetail(fields = this.table.field_list.map((field) => field.field_name)) {
    return `
        ...
        SET QUOTED_IDENTIFIER ON
        GO
        CREATE PROCEDURE [dbo].[${this.table.table_name}_GetById_${this.prefix}] @${
      this.PR.field_name
    } ${this.PR.type}
        AS
        BEGIN
            ...
            SELECT 
                ${this.table.field_list
                  .filter((field) =>
                    fields?.length > 0 ? fields.includes(field.field_name) : true
                  )
                  ?.map((field) => {
                    console.log(field.data_type);
                    return field.data_type === "datetime"
                      ? `CONVERT(VARCHAR, ${this.alias}.${field.field_name}, 103)`
  ...
  getList(fields = this.table.field_list.map((field) => field.field_name)) {
    ...
    return `
        ...
        AS BEGIN
                ...
                ${selectedFields
                  .map((field) => {
                    if (field.data_type === "datetime") {
                      return `FORMAT(VARCHAR,${this.alias}.${field.field_name}, 103) AS ${field.field_name}`;
                    } else if (field.field_name === SPECIAL_FIELDS.CREATED_USER) {
            ...
            ${
              this.table.is_deleted_field
                ? `AND ${this.alias}.${this.table.is_deleted_field.field_name} = 0`
                : ""
            } 
            ${
